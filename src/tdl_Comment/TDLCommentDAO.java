...
public class TDLCommentDAO { //MemberDAO
	...
	//
		try {
			...
			sql="select max(TPC_ref) from TDL_POST_COMMENT where TPC_num=?";//매개변수 TPC_num은 게시물번호.
			...
			pstmt.setString(4,articleC.getTPC_id()); // 작성자 아이디
...
//기존의 레코드외에 추가된 레코드를 첨부해서 다같이 보여주는 개념 -> 누적개념(벽돌) 
	...
	public String commentAddr(int TP_num) {
		...
		try {
			...
			System.out.println("TDLCommentDAO -> commentAddr()메서드 에러유발"+e);
		}finally {
			pool.freeConnection(con,pstmt,rs);
		}
		return addr;
	...
	public String commentId(int TP_num) {
		...
		try {
			...
			System.out.println("TDLCommentDAO -> commentId()메서드 에러유발"+e);
		}finally {
			pool.freeConnection(con,pstmt,rs);
		}
		return TPC_id;
