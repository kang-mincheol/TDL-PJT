...
public class TDLNoticeDAO {
	...
	//2.생성자를 통해서 연결=>의존성
	public TDLNoticeDAO() { // 생성자
		try {
			
			System.out.println("================TDLNotice - TDLNoticeDAO() 시작");
			pool=DBConnectionMgr.getInstance();
			System.out.println("pool : "+pool);
		}catch(Exception e) {
			System.out.println("DB접속오류 : "+e);
		}
		System.out.println("================ TDLNotice - TDLNoticeDAO() 끝");
	}

	//1.페이징 처리를 위해서 전체 레코드수를 구해와야 한다.		
	public int getArticleCount() {
		System.out.println("================ TDLNotice  - getArticleCount() 시작");
		int x=0;//레코드갯수
		...
		try {
			...
			System.out.println("TDLNoticeDAO 카운트 출력성공");
		}catch(Exception e) {
			System.out.println("getArticleCount()메서드 에러유발 : "+e);
		}finally {
			pool.freeConnection(con,pstmt,rs);
		}
		System.out.println("================ TDLNotice  - getArticleCount() 끝");
		return x;
	}
	...
	public List getArticles(int start, int end) {// getMemberArticle(int start,int end)
		// id asc name desc
		System.out.println("================ TDLNotice  - getArticles() 시작");
...
//형식) articleList=new 자식클래스명();
				...
				do {
					...
					TDLNoticeDTO article=makeArticleFromResult(); //중복 메서드 만들어서 불러오기
					/*
					TDLNoticeDTO article = new TDLNoticeDTO();// MemberDTO mem=new MemberDTO();
					article.setTN_num(rs.getInt("num"));//자유게시판 번호
					article.setTN_title(rs.getString("TN_title"));//자유게시판 제목
					article.setTN_id(rs.getString("TN_id"));// 유저아이디
					article.setTN_date(rs.getString("TN_date"));// 작성일
					article.setTN_content(rs.getString("TN_content")); //자유게시판 글 내용
					*/
//추가 
			...
			System.out.println("TDLNoticeDAO getArticles()메서드 에러유발" + e);
		} finally {
			pool.freeConnection(con, pstmt, rs);
		}
		System.out.println("================ TDLNotice  - getArticles() 끝");
		return articleList;
	}
	...
	public Hashtable pageList(String pageNum,int count) {
		System.out.println("================ TDLNotice  - pageList() 시작");
		...
		int pageCount=count/pageSize+(count%pageSize==0?0:1);
	       ...
	       System.out.println("================ TDLNotice  - pageList() 끝");
		return pgList;
	}
...
///////추가(1) 검색분야에 따른 검색어를 입력했을때 조건에 만족하는 레코드갯수가 필요
	...
	public int getArticleSearchCount(String search,String searchtext) { 
		System.out.println("================ TDLNotice  - getArticleSearchCount() 시작");
		int x=0;//레코드갯수
	...
	public List getTDLArticles(int start, int end,String search,String searchtext) {// getMemberArticle(int start,int end)
		...
		try {
			...
			System.out.println("TDLNotice getTDLArticles()의 sql => "+sql);
			//-----------------------------------------------------------------------------
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, start - 1);
			;// mysql은 레코드순번이 내부적으로 0부터 시작
...
//형식) articleList=new 자식클래스명();
		...
		System.out.println("================ TDLNotice  - getTDLArticles() 끝");
		return articleList;
	}
	...
	public String getTN_date() {
		System.out.println("================ TDLNotice  - getTN_date() 시작");
		...
		try {
			...
			System.out.print("TDLNoticeDAO getTN_date() 에러유발  ->");
			e.printStackTrace();
		}
		System.out.println("================ TDLNotice  - getTN_date() 끝");
		return ""; // 데이터베이스 오류
	}
	...
	//---------------게시판의 글쓰기 및 글 답변달기-------------------------------------------------------------------------
		...
		public void insertArticle(TDLNoticeDTO article) {//~(MemberDTO mem)
			System.out.println("================ TDLNotice  - insertArticle() 시작");
			//1.article -> 신규글인지 답변글인지 구분
			int TN_num=article.getTN_num();//0(신규글인지) 0이 아닌 경우(답변글)
			//테이블에 입력할 게시물 번호를 저장할 변수
			int number=0;
			System.out.println("insertArticle 메서드의 내부의 num : "+TN_num);
			
			try {
				con=pool.getConnection();
				sql="select max(TN_num) from TDL_Notice";//최대값+1=실제 저장할 게시물번호
				pstmt=con.prepareStatement(sql);
				rs=pstmt.executeQuery();
				if(rs.next()) {
				...
				System.out.println("insertArticle()의 날짜 값 ->"+getTN_date());
				String name ="test";
				...
				pstmt.setString(2,article.getTN_title());//웹에서는 Setter Method를 메모리에 저장
				pstmt.setString(3,name);
				pstmt.setString(4,getTN_date());//작성날짜
				pstmt.setString(5,article.getTN_content());//글내용
				
				int insert=pstmt.executeUpdate();
				System.out.println("TDLNoticeDAO 게시판의 글쓰기 성공유무(insert) : "+insert );
			}catch(Exception e) {
				System.out.println("TDLNoticeDAO insertArticle()메서드 에러유발 : "+e);
				e.printStackTrace();
			}finally {
				pool.freeConnection(con,pstmt,rs);
			}//finally
			System.out.println("================ TDLNotice  - insertArticle() 끝");
		}
	
	//------------업데이트-----------------------
		
		public TDLNoticeDTO getArticle(int TN_num) {
			System.out.println("================ TDLNotice  - TDLNoticeDTO getArticle() 시작");
			...
			try {
				...
				System.out.println("TDLNotice - getArticle 조회수 증가유무(update) : "+update);
			...
			System.out.println("================ TDLNotice  - TDLNoticeDTO getArticle() 끝");
			return article;
		}
	...
	//게시판의 글수정하기   업데이트
		...
		public TDLNoticeDTO updateGetArticle(int TN_num) { //updateForm.jsp에서 출력
			System.out.println("================ TDLNotice  - TDLNoticeDTO updateGetArticle() 시작");
			TDLNoticeDTO article=null;
			...
			try {
				...
				System.out.println("TDLNoticeDAO updateGetArticle()메서드 에러유발"+e);
			}finally {
				pool.freeConnection(con,pstmt,rs);
			}
			System.out.println("================ TDLNotice  - TDLNoticeDTO updateGetArticle() 끝");
			return article;
		}
	
	//  자유게시판 내용 수정시켜 저장하는 구문 ============================
		public int updateArticle(TDLNoticeDTO article) {
			System.out.println("================ TDLNotice  -  updateArticle() 시작");
			int x=-1;//게시물의 수정성공유무
			
			try {				

				System.out.println("업데이트아티클updateArticle() "+article.getTN_title());
				System.out.println("업데이트아티클updateArticle() "+article.getTN_content());
				System.out.println("업데이트아티클updateArticle() "+article.getTN_num());
				...
				int update=pstmt.executeUpdate();
				System.out.println("TDLNoticeDAO 게시판의 글수정 성공유무(update) : "+update);//1성공
				x=1;					
				
			}catch(Exception e) {
				System.out.println("TDLNoticeDAO updateArticle()메서드 에러유발 : "+e);
			}finally {
				pool.freeConnection(con,pstmt,rs);//암호를 찾기 때문에
			}
			System.out.println("================ TDLNotice  -  updateArticle() 끝");
			return x;
		}
		...
		public int deleteArticle(int TN_num) { 
			System.out.println("================ TDLNotice  -  deleteArticle() 시작");
			int x=-1;//게시물의 수정성공유무	
			try {
				con=pool.getConnection();
						...
						System.out.println("TDLNoticeDAO 게시판의 글수정 성공유무(delete) : "+delete);//1성공
						x=1;
				//if(rs.next()); -> x=-1;
			}catch(Exception e) {
				System.out.println("TDLNoticeDAO deleteArticle()메서드 에러유발 : "+e);
			}finally {
				pool.freeConnection(con,pstmt,rs);//암호를 찾기 때문에
			}
			System.out.println("================ TDLNotice  -  deleteArticle() 끝");
			return x;
		...
		private TDLNoticeDTO makeArticleFromResult() throws Exception{
			...
			TDLNoticeDTO article = new TDLNoticeDTO();// MemberDTO mem=new MemberDTO();
			article.setTN_num(rs.getInt("TN_num"));//자유게시판 번호
			article.setTN_title(rs.getString("TN_title"));//자유게시판 제목
			article.setTN_id(rs.getString("TN_id"));// 유저아이디
			article.setTN_date(rs.getString("TN_date"));// 작성일
			article.setTN_content(rs.getString("TN_content")); //자유게시판 글 내용
			article.setTN_readcount(rs.getInt("TN_readcount"));//조회수
			return article;
		}
